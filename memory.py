...
def setup_database():
    ...
    # [File] File path memory -- remembers where Duckie found every file
    cursor.execute('''
    ...
    # [Facts] Personal facts -- names, professions, preferences, etc.
    cursor.execute('''
    ...
    # [Chat] Chat history -- remembers what was said recently
    cursor.execute('''
...
def save_fact(key, value):
    ...
    conn.commit()
    conn.close()
    print(f"[Fact] Fact saved: {key} = {value}")
...
def save_file_path(nickname, full_path):
    ...
    conn.commit()
    conn.close()
    print(f"[File] File path saved: {nickname} -> {full_path}")
...
def add_chat_message(role, content):
    """Store a conversational message and prune old history."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute('INSERT INTO chat_history (role, content) VALUES (?, ?)', (role, content))
    
    # Prune: keep only last 20 messages to save memory
    cursor.execute('''
        DELETE FROM chat_history 
        WHERE id NOT IN (SELECT id FROM chat_history ORDER BY id DESC LIMIT 20)
    ''')
